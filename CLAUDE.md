...
|---|---|---|---|
| GRC system of record | CISO Assistant Community Edition (Docker) | Yes | AGPL v3, Django/SvelteKit, API-first, ~4,000 GitHub stars, actively maintained (v3.15.9 as of Apr 2026). See Stage 7 correction below — no Postgres/Redis in the actual deployment |
...
**Correction (Stage 7): no separate Postgres or Redis container exists in this deployment.** This table originally described the platform's stack as "Django/SvelteKit/PostgreSQL/Redis." The real `docker-compose.yml` in the sibling `ciso-assistant-community` checkout defines exactly six services — `backend`, `huey`, `frontend`, `qdrant`, `mcp` (defined, not running), `caddy` — confirmed via `docker ps -a` (only six containers exist, ever have) and by grepping the compose file for `DATABASE`/`POSTGRES`/`REDIS`/`BROKER` (zero matches; no `.env` file in that directory either). The deployment runs entirely on the backend image's baked-in SQLite database and a file-backed `huey` task queue, not external Postgres/Redis services. Diagrammed accurately in `docs/architecture.md`; indexed in `docs/decision-log.md` #24.
